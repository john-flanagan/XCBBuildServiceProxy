...
extension ResponsePayload: XCBProtocol.ResponsePayload {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch name {
        case "PING": self = .ping(try container.decode())
        case "BOOL": self = .bool(try container.decode())
        case "STRING": self = .string(try container.decode())
        case "ERROR": self = .error(try container.decode())
        case "SESSION_CREATED": self = .sessionCreated(try container.decode())
        case "BUILD_CREATED": self = .buildCreated(try container.decode())
        case "BUILD_PROGRESS_UPDATED": self = .buildOperationProgressUpdated(try container.decode())
        case "BUILD_PREPARATION_COMPLETED": self = .buildOperationPreparationCompleted(try container.decode())
        case "BUILD_OPERATION_STARTED": self = .buildOperationStarted(try container.decode())
        case "BUILD_OPERATION_REPORT_PATH_MAP": self = .buildOperationReportPathMap(try container.decode())
        case "BUILD_DIAGNOSTIC_EMITTED": self = .buildOperationDiagnostic(try container.decode())
        case "BUILD_OPERATION_ENDED": self = .buildOperationEnded(try container.decode())
        case "PLANNING_OPERATION_WILL_START": self = .planningOperationWillStart(try container.decode())
        case "PLANNING_OPERATION_FINISHED": self = .planningOperationDidFinish(try container.decode())
        case "BUILD_TARGET_UPTODATE": self = .buildOperationTargetUpToDate(try container.decode())
        case "BUILD_TARGET_STARTED": self = .buildOperationTargetStarted(try container.decode())
        case "BUILD_TARGET_ENDED": self = .buildOperationTargetEnded(try container.decode())
        case "BUILD_TASK_UPTODATE": self = .buildOperationTaskUpToDate(try container.decode())
        case "BUILD_TASK_STARTED": self = .buildOperationTaskStarted(try container.decode())
        case "BUILD_CONSOLE_OUTPUT_EMITTED": self = .buildOperationConsoleOutput(try container.decode())
        case "BUILD_TASK_ENDED": self = .buildOperationTaskEnded(try container.decode())
        case "INDEXING_INFO_RECEIVED": self = .indexingInfo(try container.decode())
        case "PREVIEW_INFO_RECEIVED": self = .previewInfo(try container.decode())
            
        default: self = .unknownResponse(.init(values: try container.decode([MessagePackValue].self)))
        }
    }
